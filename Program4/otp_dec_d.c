/****************************************
*	CS 344 - Program 4 Decoding Daemon
*	11/19/2017 - by (Craig) Allan Reitan
*
*	Purpose: Hosting Daemon for the Decrypting functions of Program 4
*
*****************************************/
#define _GNU_SOURCE
	...
	while(1){
		...
		if (sizeDynArr(Processes) < 6)
		{
			...
			switch(ChildPid=fork())
			{
				...
				if (strstr(buffer, "@@otp_dec@@") != NULL)
				{
					recv(establishedConnectionFD, buffer, strlen("@@otp_dec@@"), 0);
					IsTerminated = false;
					IsConnTerminated = false;
				}else{
					IsTerminated = true;
					IsConnTerminated = true;
					fprintf(stderr, "ERROR: invalid client contact otp_dec_d on port %i\n", portNumber);
				}
				...
				if (IsConnTerminated !=0)
				{
					...
					int FileEncrypted = DecryptData(SourceFP, KeyFP, ResultFP);
